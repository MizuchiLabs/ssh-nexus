<script lang="ts">
	...
	let group = $modalStore[0].meta.group;

	const updateGroup = async () => {
		try {
			await pb.collection("groups").update(group.id, group);
			showToast(toastStore, `Updated group ${group.name}`);
	...
	const deleteGroup = async () => {
		try {
			...
			showToast(toastStore, `Deleted group ${group.name}`, "error");
		} catch (error: ClientResponseError | any) {
			showToast(
				toastStore,
				error.data?.message || "Something went wrong.",
...
{#if $modalStore[0]}
	<div class="card p-4 w-modal shadow-xl space-y-4">
		...
		<footer class="modal-footer {parent.regionFooter}">
			<button class="btn variant-filled-success" on:click={updateGroup}
				>Save</button
			>
